...
import { knowledgeService as knowledgeBaseService } from '../features/knowledge/services/knowledgeService';
import type { KnowledgeItem, KnowledgeItemMetadata } from '../features/knowledge/types';
...
export const KnowledgeBasePage = () => {
  ...
  // Helper function to extract domain (same as KnowledgeTable)
  ...
  // Grouped items - use both manual groups and automatic domain grouping
  const groupedItems = useMemo(() => {
    if (viewMode !== 'grid') return [];
    
    const groups: GroupedKnowledgeItem[] = [];
    const processedItems = new Set<string>();
    
    // First, handle items with explicit group_name
    filteredItems
      .filter(item => item.metadata?.group_name)
      .forEach(item => {
        processedItems.add(item.id);
        const groupName = item.metadata.group_name!;
        const existingGroup = groups.find(g => g.title === groupName && g.id.startsWith('group_'));
    ...
    // Then, group remaining items by domain (same logic as KnowledgeTable)
    const domainGroups = new Map<string, KnowledgeItem[]>();
    ...
    filteredItems
      ...
      .forEach(item => {
        ...
        // Only group URL-based items by domain, keep file uploads separate
        if (item.metadata.source_type === 'url') {
          groupKey = extractDomain(item.url);
        } else {
          // File uploads remain ungrouped (each gets its own group)
          groupKey = `file_${item.id}`;
        }
        
        const existing = domainGroups.get(groupKey) || [];
        domainGroups.set(groupKey, [...existing, item]);
      });
    
    // Convert domain groups to GroupedKnowledgeItem objects
    Array.from(domainGroups.entries()).forEach(([domain, groupItems]) => {
      ...
      groups.push({
        id: isFileGroup ? firstItem.id : `domain_${domain.replace(/\./g, '_')}`,
        ...
        metadata: {
          ...
          // Sum up chunks and word count for grouped items
          chunks_count: groupItems.reduce((sum, item) => sum + (item.metadata.chunks_count || 0), 0),
          word_count: groupItems.reduce((sum, item) => sum + (item.metadata.word_count || 0), 0),
        },
  ...
  // No ungrouped items in grid view - all items are now grouped either manually or by domain
  const ungroupedItems = useMemo(() => {
    return []; // All items are now handled by groupedItems
  }, []);
  ...
  const handleProgressComplete = (data: CrawlProgressData) => {
    ...
    // Remove from progress items after a delay to ensure DB transaction completes
    setTimeout(() => {
      ...
      loadKnowledgeItems();
    }, 5000); // 5 second delay to ensure database transaction completion
  };
